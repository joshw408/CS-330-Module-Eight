...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (bOPressed && glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_RELEASE)
	{
		...
		// Optional: reset ortho zoom to a sane default
		gOrthoZoom = 1.0f;
	}
...
void ViewManager::PrepareSceneView()
{
	...
	// orthographic projection (auto-fit + zoom factor)
	if (bOrthographicProjection)
	{
		// Scene center (roughly between both shelves)
		const glm::vec3 sceneCenter(8.5f, 7.0f, -9.0f);

		// Use camera distance to scale ortho size so it captures everything
		float dist = glm::length(g_pCamera->Position - sceneCenter);

		// Base extents tuned for room/shelves; scaled by distance and zoom
		float orthoHeight = 17.0f * gOrthoZoom;
		float orthoWidth = orthoHeight * ((float)WINDOW_WIDTH / (float)WINDOW_HEIGHT);
